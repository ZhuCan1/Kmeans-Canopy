package com.hust.utils;

public class Config {
	//项目跟路径
//	private static String bacePath = System.getProperty("user.dir");
	//停用词文件路径
	public static String stopWordPath = "src/main/java/properties/stopwords.txt";
	//用户词表的存放路径
	public static String userWordsPath = "src/main/java/properties/userwords.txt";
	//Canopy聚类结果文件路径
	public static String CANOPY_RESULT_PATH = "F:/file/excelcrawl/cluster/canopyresult/";
	//KMeans聚类结果文件路径
	public static String KMEANS_RESULT_PATH = "F:/file/excelcrawl/cluster/kmeansresult/";
}
